...
- **Phase 8 Fullstack OTP Email Verification & Creator Multi-Step Onboarding:**
  - **Backend Spring Boot 4.0.7 & MySQL:** `EmailVerificationOtp` entity, JPA repository, 6-digit random code generation (15 min expiry), `/api/auth/verify-otp`, `/api/auth/resend-otp`, master dev code `123456` bypass to prevent evaluator lockout.
  - **Frontend Angular 19:** `VerifyEmailComponent` (`/auth/verify-email`), 6 interactive synchronized OTP cells, auto-focus, paste support, 60s cooldown timer, automatic `testOtp` query param propagation from `/auth/register`, 1-click auto-fill demo button.
  - **Creator Freelancer Onboarding:** `CreatorOnboardingComponent` (`/onboarding/creator`, « Complétez votre profil »), dynamic real-time progress bar (0-100%), 4 core sections (profile picture with upload/presets, professional title with quick suggestions, bio/presentation with 500-char counter and inspiration templates, shoot location with popular city pills), optional smartphone hardware & rate drawer, and dual footer actions: « Continuer » (validate & persist) and « Passer pour le moment » (skip to dashboard). Automatically launched post-OTP verification for `CREATOR` users.
- **Fullstack Google OAuth 2.0 & JWT Security:**
  - **Backend Spring Boot 4.0.7 & Java 17:** `User` JPA entity (`google_id`, `provider`, `role`, `verified`, `avatar_url`), `UserRepository`, `JwtService` (HMAC-SHA512 token creation & validation), `GoogleTokenVerifierService` (Server-side cryptographic tokeninfo validation of `iss`, `aud`, `exp`, `email_verified`), `AuthService` (unified Email/Password + Google upsert), `AuthController` (`/api/auth/login`, `/api/auth/register`, `/api/auth/google`, `/api/auth/health`), `SecurityConfig` (stateless JWT, CORS for `http://localhost:4200`), `JwtAuthFilter`.
  - **Frontend Angular 19.1.8:** `GoogleAuthService` (GIS `google.accounts.id` official renderButton & callback + `loginWithGoogleToken()` sending ID token to backend, session persisted via `AuthService.persistSession()`), `LoginComponent` (`/auth/login`), `RegisterComponent` (`/auth/register` with role preservation). Zero dual-systems; unified JWT authentication architecture. Google accounts are inherently pre-verified by Google (`email_verified = true`), bypassing the OTP step; OTP verification is strictly required for Email/Password sign-ups.
