...
- [x] **Authentification Sociale Google OAuth (✅ Terminé) :** Intégration officielle Google Identity Services (GIS `google.accounts.id`), bouton avec rendu GIS officiel et fallback guidé, vérification cryptographique des tokens Google côté backend Spring Boot (`GoogleTokenVerifierService`), création automatique de compte et persistance de session JWT.
- [x] **Backend REST Spring Boot & Sécurité JWT (✅ Terminé) :** Endpoints `/api/auth/{login,register,google,forgot-password,verify-reset-token,reset-password,health}`, filtres `JwtAuthFilter`, `SecurityConfig` CORS/Stateless, base de données MySQL et gestion des exceptions centralisée.
- [x] **Système Complet "Mot de passe oublié" & "Réinitialisation" (✅ Terminé) :** Implémentation de l'entité JPA `PasswordResetToken`, repository, DTOs, services backend, composant Angular `ForgotPasswordComponent` (glassmorphic, validation, chips de démo) et `ResetPasswordComponent` (vérification de token, jauge dynamique de complexité de mot de passe, visibilité 👁️, validation de correspondance et redirection automatique). Testé et validé à 100% de bout en bout en navigateur.
...
- [x] **Phase 8 Système de Vérification d'Email OTP & Onboarding Créateur Multi-Étapes (✅ Terminé) :**
  - **Backend Spring Boot :** Entité JPA `EmailVerificationOtp`, table MySQL `email_verification_otps`, génération de code sécurisé à 6 chiffres (validité 15 min), endpoints `/api/auth/register`, `/api/auth/verify-otp`, `/api/auth/resend-otp`, support du code universel de test `123456` pour les évaluations et bypass automatique des blocages dev.
  - **Frontend Angular 19 :** Composant interactif `VerifyEmailComponent` (`/auth/verify-email`), synchronisation directe avec les éléments `<input>` natifs du DOM, auto-focus progressif, navigation par flèches et `Backspace`, support du collage/paste de 6 chiffres, compte à rebours 60s pour renvoi de code, transmission dynamique de `testOtp` en query params et bouton *« Remplir automatiquement »*.
  - **Onboarding Freelancer / Créateur « Complétez votre profil » (`/onboarding/creator`) :**
    - Redirection automatique déclenchée après la vérification OTP pour le rôle `CREATOR`.
    - **Barre de progression dynamique** réactive en temps réel (0% à 100%) calculée sur la complétion des 4 sections fondamentales :
      1. *Photo de profil* : Prévisualisation nette, upload de fichier JPG/PNG/WEBP, bouton caméra overlay 📷, avatar aléatoire 🎲 et sélection de presets d'avatars.
      2. *Titre professionnel* : Input avec 4 suggestions rapides cliquables (*Vidéaste Mobile 4K & UGC, Photographe ProRes...*).
      3. *Présentation / Bio* : Textarea avec compteur interactif (0/500 caractères) et modèles d'inspiration rapides.
      4. *Ville & Région* : Input avec icône 📍 et suggestions de métropoles (*Paris, Lyon, Tunis, Bruxelles...*).
    - **Volet Optionnel Dépliable :** Configuration optionnelle du modèle de smartphone (iPhone 16 Pro Max ProRes, Galaxy S24 Ultra, Pixel 9 Pro) et des tarifs journalier/horaire.
    - **Double Action Ergonomique :**
      - Bouton *« Continuer »* : Validation, persistance des données dans `snapconnect_user` et `snapconnect_creator_profile_data`, mise à jour du signal réactif utilisateur et redirection vers `/creator/dashboard`.
      - Bouton *« Passer pour le moment »* : Permet d'ignorer l'onboarding pour explorer immédiatement la plateforme.
  - **Tests & Validation :** Testé avec succès de bout en bout en navigateur réel (Inscription ➔ OTP ➔ Complétion profil 100% ➔ Dashboard créateur).
